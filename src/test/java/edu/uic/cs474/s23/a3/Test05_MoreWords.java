...
public class Test05_MoreWords {

    @Test
    public void test01() {
        StreamProcessor processor = Main.getProcessor();

        String[] words1 = { "words", "and", "more", "words" };
        String[] words2 = { "anything", "that", "appears", "here", "appears", "nowhere", "else" };
    ...
    public void testRandom() {
        ...
        for (int i = 0 ; i < 10 ; i++) {
            list1.add(source[r.nextInt(source.length)] + "1");
            list2.add(source[r.nextInt(source.length)] + "2");
            list3.add(source[r.nextInt(source.length)] + "3");
        }
